...
def generate_notes_from_content(book_text):
    ...
    connect_all_json = ",".join(cleaned_response)  # This variable will combine all the items of the list making it a single json array

    # print(f"\nCleaned Json response: {connect_all_json}")


    full_json_array = "[" + connect_all_json + "]"

    # print(f"\nAll Json Combined: {full_json_array}")



    final_json_array = validate_and_fix_json(full_json_array)

    # print(f"\nFinal Json Combined: {final_json_array}")

    json_str = json.dumps(final_json_array)

    generated_json_for_word = json.loads(json_str)
    #
    # print(f"\nGenerated Json for Word Doc: {generated_json_for_word}")
    #
    # print(f"\nFinal Json for word File: {generated_json_for_word}")
